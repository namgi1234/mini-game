...
Vector2.prototype.intersects = function (obj) {
    return (
        obj && obj.x !== undefined && obj.y !== undefined && obj.x < this.x + this.width && obj.y < this.y + this.height && obj.x + obj.width > this.x && obj.y + obj.height > this.y
    );
};

Vector2.prototype.intercectsLeft = function (obj) {
...
Player.prototype.update = function () {
    ...
    if (this.y > InfiniteRunner.height || this.x + this.width < 0) {
        if (pt == 1) {
            if (getCookie('jump') < InfiniteRunner.jumpCount) {
                document.cookie = "jump=" + InfiniteRunner.jumpCount
            }
            alert("GAME OVER!!")
        ...
        InfiniteRunner.scoreColor = 'wheat';
        InfiniteRunner.platformManager.maxDistanceBetween = 350;
        InfiniteRunner.platformManager.updateWhenLose();
    }

    if ((InfiniteRunner.keys.S || InfiniteRunner.keys.ENTER || InfiniteRunner.keys.W || InfiniteRunner.dragging) && this.velocityY < -8) {
        this.velocityY += -0.75;
    }
...
PlatformManager.prototype.update = function () {
    ...
    if (this.first.x + this.first.width < 0) {
        ...
        this.first.x = (this.third.x + this.third.width) + random(
            this.maxDistanceBetween - 150,
            this.maxDistanceBetween
        );
    ...
    if (this.second.x + this.second.width < 0) {
        ...
        this.second.x = (this.first.x + this.first.width) + random(
            this.maxDistanceBetween - 150,
            this.maxDistanceBetween
        );
...
InfiniteRunner.setup = function () {
    ...
    this.scoreColor = 'wheat';
    this.jumpCountRecord = getCookie('jump');
...
InfiniteRunner.update = function () {
    ...
    for (i = 0; i < this.platformManager.platforms.length; i++) {
        if (this.player.intersects(this.platformManager.platforms[i])) {
            ...
            if (this.player.intercectsLeft(this.platformManager.platforms[i])) {
